...
const PresEdit = ({route,navigation }) => {
    const [nombre, setNombre] = useState(route.params.data.PresentacionNombre)
    const [descripcion, setDescripcion] = useState(route.params.data.PresentacionDescripcion)
    ...
    const pressGuardar = async () => {
        try {
            ...
            const response = await fetch('http://192.168.0.2:7777/api/presentacion/updateP', {
                method: 'PUT',
                headers: {
